...
# it under %LOCALAPPDATA%\Programs\jabari\bin with the jabari icon, adds that
# directory to your user PATH, and creates a Start Menu shortcut — so jabari
...
#
# Options:
#   $env:JABARI_VERSION    pinned release tag, e.g. v1.0.0 (default: latest)
#   $env:JABARI_PREFIX     install directory (default: %LOCALAPPDATA%\Programs\jabari)
...
$Bin = "jabari"
$Version = $env:JABARI_VERSION
$Prefix = $env:JABARI_PREFIX
if (-not $Prefix) {
    $Prefix = Join-Path $env:LOCALAPPDATA "Programs\jabari"
}
...
function Build-FromSource {
    ...
    if (-not (Test-Path (Join-Path $PWD "go.mod"))) {
        ...
        $tmp = Join-Path $env:TEMP "qyvora-jabari-src"
        if (Test-Path $tmp) { Remove-Item -Recurse -Force $tmp }
        Write-Step "cloning QYVORA/qyvora-jabari..."
        git clone --depth 1 "https://github.com/QYVORA/qyvora-jabari" $tmp | Out-Null
        Set-Location $tmp
    }
    $go = Get-Command go -ErrorAction SilentlyContinue
    if (-not $go) { Write-Err "go is required to build from source"; exit 1 }
    go build -trimpath -ldflags "-s -w" -o (Join-Path $Dest "$Bin.exe") ./cmd/jabari
...
function Install-Icon {
    ...
    if (-not (Test-Path $ico)) {
        try {
            ...
            $candidates = @(
                (Join-Path $PWD "assets\jabari.ico"),
...
function Install-StartMenuShortcut {
    ...
    $sc.IconLocation = $ico
    $sc.Description = "AI-assisted Android and web penetration testing framework"
    $sc.Save()
...
Write-Result "run 'jabari --help' to get started."
