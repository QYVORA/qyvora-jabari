...
banner() {
    ...
    echo -e "${CYAN}           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%          "
    echo -e "${NC}"
    echo -e "  ${WHITE}JABARI — Zero-Config Installer${NC}"
...
install_release() {
    ...
    if [ "$os" != "windows" ]; then
        if curl -fsSL --connect-timeout 10 "${BASE_URL}/jabari.png" -o "${WORK_DIR}/jabari.png"; then
            if [ -n "$hashtool" ] && [ -f "${WORK_DIR}/checksums.txt" ]; then
                local cwant cgot
                cwant=$(awk -v n="jabari.png" '$2 == n { print $1 }' "${WORK_DIR}/checksums.txt")
                if [ -n "$cwant" ]; then
                    cgot=$($hashtool "${WORK_DIR}/jabari.png" | awk '{ print $1 }')
                    if [ "$cwant" = "$cgot" ]; then
                        ok "SHA-256 verified for jabari.png"
                    else
                        warn "Icon checksum mismatch; desktop icon will be skipped."
                        rm -f "${WORK_DIR}/jabari.png"
                    fi
                fi
            fi
        else
            warn "Could not fetch jabari.png; desktop icon will be skipped."
        fi
    fi
...
install_from_source() {
    ...
    if [ -f "$PWD/go.mod" ]; then
        ...
        if CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o "${WORK_DIR}/jabari-src" ./cmd/jabari; then
            INSTALL_BIN="${WORK_DIR}/jabari-src"
            return 0
        else
    ...
    if ! (cd "${WORK_DIR}/src" && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o "${WORK_DIR}/jabari-src" ./cmd/jabari); then
...
install_binary() {
    ...
    if [ "$dest" = "/usr/local/bin" ] && [ ! -w "$dest" ]; then
        if sudo -n true 2>/dev/null; then
            sudo -n install -m 0755 "$INSTALL_BIN" "$dest/jabari"
        else
    ...
    mkdir -p "$dest"
    if [ -w "$dest" ]; then
        install -m 0755 "$INSTALL_BIN" "$dest/jabari"
...
configure_path() {
    ...
    printf '\n# added by JABARI installer\n%s\n' "$line" >> "$rc"
    warn "Added ${dest} to your PATH in ${rc}"
    warn "Run: source ${rc}  (or open a new terminal) to use 'jabari'"
}

# ---------------------------------------------------------------------------
# Desktop integration (Linux only): install the app icon and a .desktop entry
# so jabari shows up with its logo in GNOME's search/app grid. Data goes next
...
install_desktop_integration() {
    ...
    local icon_src="${WORK_DIR}/jabari.png"
    if [ -f "$PWD/assets/jabari.png" ]; then
        icon_src="$PWD/assets/jabari.png"
    elif [ ! -f "$icon_src" ] && [ -f "${WORK_DIR}/src/assets/jabari.png" ]; then
        icon_src="${WORK_DIR}/src/assets/jabari.png"
    fi
    ...
    install -m 0644 "$icon_src" "$icon_dir/jabari.png"

    local desktop="$apps_dir/jabari.desktop"
    if [ ! -f "$desktop" ]; then
        cat >"$desktop" <<EOF
[Desktop Entry]
Type=Application
Name=JABARI
GenericName=Android Security Assessment Framework
Comment=Android security assessment: USB and network (ADB) targets, seven-stage pipeline
Exec=$dest/jabari
Icon=$icon_dir/jabari.png
Terminal=true
Categories=Utility;Development;Security;
Keywords=android;adb;security;assessment;audit;poc;apk;device;rules;
EOF
    fi
    chmod 0644 "$desktop"
    ok "Installed desktop entry: ${desktop}"
    ok "Installed app icon: ${icon_dir}/jabari.png"
...
verify() {
    local dest="$1"
    local bin="$dest/jabari"

    if [ ! -x "$bin" ]; then
        warn "jabari binary not found at ${bin}."
        return 1
    fi

    info "Verifying installation..."
    "$bin" version 2>/dev/null || "$bin" --help >/dev/null 2>&1
    ok "JABARI installed at ${bin}"
    ok "Run 'jabari --help' to get started."
    return 0
}
